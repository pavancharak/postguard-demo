...
post's id — only its `text` and `timestamp`. Change the text, change the outcome.

## Why

The point isn't "this app has a checkmark." It's that the checkmark is **backed by
...
authorization/signing/audit split as three separate, swappable pieces instead of one
...
1. **Drafts** — a raw, unverified post from an AI agent (`src/data.js` — no pass/fail baked in, on purpose).
...
3. **Receipt** (`src/components/ReceiptScreen.jsx`) — on mount, it:
   - calls `policyEngine.evaluate(post)` — real rule evaluation, happens here, not upstream
...
4. **Try to Override** — this button doesn't fake a rejection message. It calls
   ...
   now-mutated record and compares it to the signature that was computed *before* the
   edit. Mismatch → **"Tampering detected."** That's a computed result, not a string
   someone typed into the button handler.
...
- **Deterministic authorization** — the same post + rules always produces the same
  verdict, so "why was this blocked" always has a real answer (`failures[]`).
- **Non-repudiable decisions** — once signed, a decision can't be silently changed without
  the signature breaking, which is a stronger guarantee than an audit log that just trusts
  its own rows.
...
Try it: open DevTools → Application → Local Storage → `audit_trail`, change any field in a
record, then hit "Verify" on that record in the Audit Trail screen. It will report a
signature mismatch — this is the same code path "Try to Override" uses, just triggered
manually instead of through the UI button.
